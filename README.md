# FUAliShortVideoDemo 快速接入文档

FUAliShortVideoDemo 是集成了 [Faceunity](https://github.com/Faceunity/FULiveDemo/tree/dev) 面部跟踪和虚拟道具功能 和 阿里短视频 功能的 Demo。

本文是 FaceUnity SDK 快速对接阿里短视频的导读说明，关于 FaceUnity SDK 的更多详细说明，请参看 [FULiveDemo](https://github.com/Faceunity/FULiveDemo/tree/dev)

注：本例是示例 Demo ,仅在 首页 --> 拍摄 --> 启动录制中 加入了 FaceUnity 效果，如需更多接入用户可参考 FUAliShortVideoDemo。
...
在 AliyunRecordViewController.m 的 `viewDidLoad:` 方法中调用快速加载道具函数，该函数会创建一个美颜道具及指定的贴纸道具。
...
遵循 `AliyunIRecorderDelegate` 代理，并在其代理方法 `recorderOutputVideoRawSampleBuffer:` 中进行图像处理：
...
1、在 AliyunRecordViewController.m 中添加头文件，并创建 demoBar 属性
