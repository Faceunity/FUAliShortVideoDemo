    ...
    if (_cutInfo.phImage) {
        ...
        [[PHImageManager defaultManager] requestImageForAsset:_cutInfo.phAsset targetSize:PHImageManagerMaximumSize contentMode:PHImageContentModeDefault options:requestOptions resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
            if (result.imageOrientation == UIImageOrientationUp)  { //fix:iOS8下图片会旋转的问题
                self->_stillImage = result;
                self->_stillImageLayer.contents = (__bridge id _Nullable)(result.CGImage);
            } else {
                ...
                self->_stillImage = normalizedImage;
                self->_stillImageLayer.contents = (__bridge id _Nullable)(normalizedImage.CGImage);
            }
        }];
    }
...
- (void)cropOnError:(int)error {
    ...
    if (_isCancel || error == ALIVC_SVIDEO_ERROR_TRANSCODE_BACKGROUND) {
        ...
        _hasError = YES;
//        NSString *err = [NSString stringWithFormat:@"%@ %d",NSLocalizedString(@"错误码:", nil),error];
//        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"裁剪失败", nil) message:err delegate:nil cancelButtonTitle:NSLocalizedString(@"知道了", nil) otherButtonTitles:nil, nil];
//        [alert show];
...
- (void)didStartClip {
    ...
    if (_cutInfo.cutMode == 1||!self.fakeCrop) {
        ...
        //只有画幅变了才是真裁剪 其他都是假裁剪
        if(!CGSizeEqualToSize(_previewScrollView.bounds.size, _previewScrollView.contentSize)||!self.fakeCrop) {
...
- (void)onClickCropButton {
    ...
    self.previewScrollView.scrollEnabled = NO;
}
