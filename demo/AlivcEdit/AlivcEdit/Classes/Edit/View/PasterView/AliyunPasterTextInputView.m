...
//    self.closeButton.center = CGPointMake(0, 0);
    ...
    self.textView.font = [UIFont systemFontOfSize:PasterInputViewFontSize];
...
-(void)removeStrokeColor{
    _textView.mTextStrokeColor = [UIColor clearColor];
}
...
{
    _textActoinType = type;
}

- (TextActionType)actionType
{
    return _textActoinType;
}
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    if (fixedWidth >= self.maxWidth) {
        fixedWidth = self.maxWidth;
    }
    ...
    if (textView.markedTextRange == nil) {//刷新文字
        if ([self textLength:text] <=self.maxCharacterCount) {//对文字数目没有要求
            textView.text = text;
        } else {
