...
#import "AlivcRaceButton.h"
#import "UIView+AlivcHelper.h"
#import "UIColor+AlivcHelper.h"

//高度
static CGFloat AlivcRaceBottomMenuHeaderViewHeight = 45;

@implementation AlivcRaceBottomMenuHeaderViewItem
-(instancetype)init{
    self =[super init];
    if (self) {
        _title =@"";
...
+(instancetype)createItemWithTitle:(NSString *)title icon:(UIImage *)icon tag:(NSInteger)tag{
    AlivcRaceBottomMenuHeaderViewItem *item =[AlivcRaceBottomMenuHeaderViewItem new];
    item.title = title;
    item.icon = icon;
    item.tag = tag;
    return item;
...
@implementation AlivcRaceBottomMenuHeaderView
{
    UIView *_flagView;
}

-(instancetype)initWithItems:(NSArray<AlivcRaceBottomMenuHeaderViewItem *> *)items{
    self = [super initWithFrame:CGRectMake(0, 0, ScreenWidth, AlivcRaceBottomMenuHeaderViewHeight)];
    if (self) {
...
-(void)setupSubviewsItems:(NSArray<AlivcRaceBottomMenuHeaderViewItem *> *)items{
