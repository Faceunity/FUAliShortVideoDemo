...
@interface AlivcRaceLiveBeautifyDetailItemView : UIView
+ (instancetype)itemViewWithText:(NSString *)text image:(UIImage *)image action:(void(^)(AlivcRaceLiveBeautifyDetailItemView *sender))action;
@property (nonatomic, assign) BOOL selected;
@end

@implementation AlivcRaceLiveBeautifyDetailItemView {
    UIButton *_button;
    UILabel *_textLabel;
    void(^_action)(AlivcRaceLiveBeautifyDetailItemView *sender);
}

- (instancetype)initWithText:(NSString *)text image:(UIImage *)image action:(void(^)(AlivcRaceLiveBeautifyDetailItemView *sender))action {
    if (self = [self initWithFrame:CGRectZero]) {
        ...
        [_button setBackgroundImage:[AlivcImage imageNamed:@"bg_btn_image"] forState:UIControlStateNormal];
        [_button setBackgroundImage:[AlivcImage imageNamed:@"bg_btn_image_selected"] forState:UIControlStateNormal | UIControlStateSelected];
...
@implementation AlivcRaceLiveBeautifyDetailView {
    AlivcRaceLiveBeautifyNavigationView *_navigationView;
    NSArray<AlivcRaceLiveBeautifyDetailItemView *> *_itemViews;
    UIScrollView *_buttonsContentView;
    __weak AlivcRaceLiveBeautifyDetailItemView *_selectedView;
    NSArray *_items;
}
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _navigationView = [AlivcRaceLiveBeautifyNavigationView navigationViewTitleView:titleLabel];
...
//        _buttonsContentView.backgroundColor =[UIColor redColor];
...
- (NSArray<AlivcRaceLiveBeautifyDetailItemView *> *)_itemViewsWithInfo:(NSArray *)info {
    __weak typeof(self) weakSelf = self;
    NSMutableArray<AlivcRaceLiveBeautifyDetailItemView *> *array = [NSMutableArray arrayWithCapacity:info.count];
    for (NSDictionary *item in info) {
        UIImage *image = [AlivcImage imageNamed:item[@"icon_name"]];
        AlivcRaceLiveBeautifyDetailItemView *view =
            [AlivcRaceLiveBeautifyDetailItemView itemViewWithText:item[@"title"]
                                                         image:image
                                                        action:^(AlivcRaceLiveBeautifyDetailItemView *sender) {
...
- (void)setDefaultValue{
    if (_itemViews.count) {
        AlivcRaceLiveBeautifyDetailItemView *firstView = _itemViews.firstObject;
        [self _aliDidSelectedView:firstView];
    }
}

- (void)_aliDidSelectedView:(AlivcRaceLiveBeautifyDetailItemView *)sender {
...
- (AlivcRaceLiveBeautifyNavigationView *)navigationView {
...
- (void)layoutSubviews {
    ...
    if (_items.count > 5) {
        ...
        for (AlivcRaceLiveBeautifyDetailItemView *view in _itemViews) {
