...
@property(nonatomic, strong)UILabel *lastCell;
...
//    AliyunEffectFontInfo *systemFont = [[AliyunEffectFontInfo alloc]init];
//    systemFont.fontName = @"系统字体";
//    [self.dataSource addObject:systemFont];
    // 获取字体
...
-(void)btnAction:(UIButton *)btn{
    if ([btn.currentTitle isEqualToString:@"简约"]) {
        
    }else if ([btn.currentTitle isEqualToString:@"手写"]){
...
-(void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
    ...
    _lastCell.textColor = [UIColor whiteColor];
    _lastCell.font = [UIFont systemFontOfSize:18];
    _lastCell = lab;
    if (self.delegate && [self.delegate respondsToSelector:@selector(onSelectFontWithFontInfo:)]) {
        [self.delegate onSelectFontWithFontInfo:self.dataSource[row]];
    }
