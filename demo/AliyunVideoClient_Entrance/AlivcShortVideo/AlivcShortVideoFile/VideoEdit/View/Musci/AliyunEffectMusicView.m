...
- (void)layoutSubviews {
    ...
    int coverHeight = 120;
    self.controlView.frame = CGRectMake(0, 0, width, 120);
    self.tableview.frame = CGRectMake(0, coverHeight, width, height - coverHeight - 48);
    self.tabView.frame = CGRectMake(0, height-48, width, 48);
    self.noneDataLabel.frame = CGRectMake(0, coverHeight, width, height- coverHeight - 48);
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // 更新音乐
    if (self.delegate && [self.delegate respondsToSelector:@selector(musicViewDidUpdateMusic:startTime:duration:streamStart:streamDuration:)]) {
        if (resourcePath) {
            AVAsset *asset = [AVAsset assetWithURL:[NSURL fileURLWithPath:[NSHomeDirectory() stringByAppendingPathComponent:resourcePath]]];
            float assetDuration = [asset avAssetAudioTrackDuration];
            CGFloat start = self.controlView.sliderTime.selectedMinimum * assetDuration / 100.0;
            CGFloat duration = (self.controlView.sliderTime.selectedMaximum-self.controlView.sliderTime.selectedMinimum ) * assetDuration / 100.0;
            CGFloat streamStart = self.controlView.sliderStreamTime.selectedMinimum / 100.0;
            CGFloat streamDuration = (self.controlView.sliderStreamTime.selectedMaximum-self.controlView.sliderStreamTime.selectedMinimum ) / 100.0;
            [self.delegate musicViewDidUpdateMusic:[NSHomeDirectory() stringByAppendingPathComponent:resourcePath] startTime:start duration:duration streamStart:streamStart streamDuration:streamDuration];
        }else {
            [self.delegate musicViewDidUpdateMusic:nil startTime:0 duration:0 streamStart:0 streamDuration:0];
        }        
    }
}
