...
//TODO:此类需再抽一层,否则会太庞大
@interface AliyunEditViewController () <AliyunIExporterCallback, AliyunIPlayerCallback, AliyunICanvasViewDelegate ,AliyunPaintingEditViewDelegate,AliyunMusicPickViewControllerDelegate,AliyunPasterBottomBaseViewDelegate,AliyunEffectCaptionShowViewDelegate,AliyunEffectTransitionViewDelegate>
...
@property (nonatomic, strong) AliyunEffectFilterView *specialFilterView;
...
/**
 记录纯字幕特效状态，用来提示倒播不支持字幕特效，SDK修复倒播不兼容字幕特效的BUG后 删除此值及提示逻辑
 */
...
- (void)addSubviews {
    ...
    self.editButtonsView = [[AliyunEditButtonsView alloc]initWithConfig:self.uiConfig];
    self.editButtonsView.frame = CGRectMake(0, ScreenHeight - 70 - SafeBottom, ScreenWidth, 70);
...
- (void)addWatermarkAndEnd{
    ...
    if (_config.hasEnd) {
        ...
        tailWatermark.frame = CGRectMake(_outputSize.width / 2 - 84 / 2, _outputSize.height / 2 - 60 / 2, 84, 60);
...
-(AliyunTimelineView *)getOneTimeLineView{
    ...
    //缩略图
    AliyunTimelineView *timeLineView = [[AliyunTimelineView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, 32)];
...
- (UIButton *)playButton{
    if (!_playButton) {
        ...
        [_playButton setBackgroundColor:[UIColor colorWithRed:0 green:0 blue:0 alpha:0.5]];
...
- (AliyunPasterShowView *)pasterShowView {
    if (!_pasterShowView) {
        _pasterShowView = [[AliyunPasterShowView alloc] initWithFrame:(CGRectMake(0, ScreenHeight, ScreenWidth, 200))];
...
- (AliyunEffectCaptionShowView *)captionShowView {
    if (!_captionShowView) {
        _captionShowView = [[AliyunEffectCaptionShowView alloc] initWithFrame:CGRectMake(0, ScreenHeight, ScreenWidth, 200)];
...
- (AliyunEffectFilterView *)filterView {
    if (!_filterView) {
        _filterView = [[AliyunEffectFilterView alloc] initWithFrame:CGRectMake(0, ScreenHeight, ScreenWidth, 180)];
...
//特效
- (AliyunEffectFilterView *)specialFilterView {
    if (!_specialFilterView) {
        _specialFilterView = [[AliyunEffectFilterView alloc] initWithFrame:CGRectMake(0, ScreenHeight, ScreenWidth, 220)];
        _specialFilterView.delegate = (id<AliyunEffectFilter2ViewDelegate>)self;
        [self.view addSubview:_specialFilterView];
        [_specialFilterView addVisualEffect];
        [_specialFilterView reloadDataWithEffectType:7];
...
- (AliyunPaintingEditView *)paintShowView {
    if (!_paintShowView) {
        _paintShowView = [[AliyunPaintingEditView alloc] initWithFrame:CGRectMake(0, ScreenHeight, ScreenWidth, 185)];
...
- (AliyunICanvasView *)paintView{
    if (!_paintView) {
        ...
        _paintView = [[AliyunICanvasView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, self.movieView.frame.size.height) paint:paint];
    ...
    if (!_transitionView) {
        ...
        for (int idx = 0; idx < clips.count; idx++) {
            ...
            if (clip.mediaType == AliyunClipImage) {
                ...
                UIImage *image = [AliAssetImageGenerator thumbnailImageForVideo:[NSURL fileURLWithPath:clip.src] atTime:0.001];
                [images addObject:image];
            }
        }
        _transitionView = [[AliyunEffectTransitionView alloc] initWithFrame:CGRectMake(0, ScreenHeight, ScreenWidth, 200) delegate:self];
        __weak typeof(self)weakSelf = self;
        [_transitionView setupDataSourceClips:images blockHandle:^(NSArray<AliyunTransitionCover *> *covers, NSArray<AliyunTransitionIcon *> *icons) {
...
- (UIButton *)saveButton{
    if (!_saveButton) {
        ...
        [_saveButton setTitle:[@"保存" localString] forState:UIControlStateSelected];
        [_saveButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [_saveButton setTitleColor:[UIColor whiteColor] forState:UIControlStateSelected];
        [_saveButton addTarget:self action:@selector(apply) forControlEvents:UIControlEventTouchUpInside];
    }
    return _saveButton;
}
...
- (UIButton *)cancelButton{
    if (!_cancelButton) {
        ...
        [_cancelButton setTitle:[@"取消" localString] forState:UIControlStateSelected];
...
- (UIButton *)backButton{
    if (!_backButton) {
        ...
        [_backButton setTitleColor:[UIColor whiteColor] forState:UIControlStateSelected];
...
- (UIButton *)publishButton{
    if (!_publishButton) {
        ...
        [_publishButton setImage:_uiConfig.finishImage forState:UIControlStateNormal];
        [_publishButton setImage:_uiConfig.finishImage forState:UIControlStateSelected];
        [_publishButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [_publishButton setTitleColor:[UIColor whiteColor] forState:UIControlStateSelected];
...
- (void)cancel{
    ...
    if (_editSouceClickType == AliyunEditSouceClickTypePaster  || _editSouceClickType == AliyunEditSouceClickTypeCaption){
        [self.editZoneView.currentPasterView removeFromSuperview];
        self.editZoneView.currentPasterView = nil;
        if(_lastPasterController){
//            [self.pasterManager deletePasterController:_lastPasterController];
...
- (void)publish{
    ...
    vc.backgroundImage = _currentTimelineView.coverImage;
...
- (void)resourceDeleteNoti:(NSNotification *)noti {
    ...
    for(int i = (int)pasterList.count -1 ; i>=0 ; i--){
        ...
        if ([[controller getIconPath] containsString:contrastStr]) {
            NSLog(@"删除动图");
            [weakSelf deletePasterController:controller];
        }
    }
    ...
    if (model.effectType == AliyunEffectTypeMV) {
        ...
        [self.mvView reloadDataWithEffectType:AliyunEffectTypeMV];
    }
}
...
- (void)applicationWillResignActive {
    if (self.isAppear) {
        ...
        // 特效正在添加过程中
        if (_processAnimationFilter && _editSouceClickType == AliyunEditSouceClickTypeEffect){
...
#pragma mark贴图相关操作
//删除单个动图
-(void)deletePasterController:(AliyunPasterController *)paster{
    if (paster) {
...
//            [MBProgressHUD showMessage:[NSString stringWithFormat:@"播放错误,错误码:%d",returnValue] inView:self.view];
        }
...
- (void)resume{
    if (self.player.isPlaying) {
        ...
        if (returnValue == 0) {
            ...
            [self.player play];
//            [MBProgressHUD showMessage:[NSString stringWithFormat:@"继续播放错误,错误码:%d",returnValue] inView:self.view];
...
//            [MBProgressHUD showMessage:[NSString stringWithFormat:@"暂停错误,错误码:%d",returnValue] inView:self.view];
...
/**
 ...
 @param type 动作类型
 */
- (void)enterEditWithActionType:(AliyunEditSouceClickType)type animationCompletion:(void (^ __nullable)(BOOL finished))completion{
...
//        [self.currentTimelineView setActualDuration:[self.player getDuration]];
...
/**
 ...
 @param type 编辑类型
 */
- (void)quitEditWithActionType:(AliyunEditSouceClickType)type CompletionHandle:(void (^ __nullable)(BOOL finished))completion{
    ...
    UIView *view = [self editViewWithType:type];
    if (view) {
        [self p_dismissEffectView:view duration:animationTime CompletionHandle:completion];
    }
...
- (BOOL)isEditFrameType:(AliyunEditSouceClickType)type{
    if (type == AliyunEditSouceClickTypeMusic || type == AliyunEditSouceClickTypePaster || type == AliyunEditSouceClickTypeCaption  || type == AliyunEditSouceClickTypeEffect || type == AliyunEditSouceClickTypeTimeFilter || type == AliyunEditSouceClickTypeTranslation || type == AliyunEditSouceClickTypePaint) {
        return YES;
    }
    return NO;
}
...
/**
 ...
 @param duration 动画展示所需的时间
 */
- (void)p_dismissEffectView:(UIView *)view duration:(CGFloat)duration CompletionHandle:(void (^ __nullable)(BOOL finished))completion{
    [UIView animateWithDuration:duration animations:^{
        CGRect f = view.frame;
...
- (void)p_changeUIToEnterEditFrameModeCompletionHandle:(void (^ __nullable)(BOOL finished))completion{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        ...
        [self.alivcPasterManager correctedPasterFrameAtEditStatusWithPasterManager:self.pasterManager withEditFrame:editFrame];
        if (completion) {
            completion(finished);
        }
    }];
...
- (void)p_setMovieViewFrameToPlayStatusWithAnimation{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        ...
        if (finished) {
            ...
            self.pasterManager.displaySize = self.editZoneView.bounds.size;
            //修正由于编辑区域变化引起的精度偏差从而导致动图位置偏移的BUG,如果pasterManager.displaySize一直没变则不需要进行此处理
            [self.alivcPasterManager correctedPasterFrameAtPreviewStatusWithPasterManager:self.pasterManager];
            if (_tabController) {
                _tabController = nil;
            }
            if (self.paintView) {
...
//    NSLog(@"playSec%f   ------   streamSec%f",(float)playSec,(float)streamSec);
}
...
- (void)playError:(int)errorCode {
    ...
    NSString *errString = [NSString stringWithFormat:@"播放错误,错误码:%ld",(long)errorCode];
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:errString message:nil delegate:nil cancelButtonTitle:NSLocalizedString(@"video_affirm_common", nil) otherButtonTitles:nil, nil];
    [alert show];
...
- (void)removePasterFromTimeline:(AliyunPasterController *)pasterController {
    AliyunTimelineItem *timeline = [self.currentTimelineView getTimelineItemWithOjb:pasterController];
    if (pasterController.pasterType == AliyunPasterEffectTypeNormal) {
...
- (void)currentTouchPoint:(CGPoint)point {
    ...
    if (pasterController) {//如果当前点击位置上存在一个动图
        ...
        if (pasterController.pasterType == AliyunPasterEffectTypeNormal && _editSouceClickType != AliyunEditSouceClickTypePaster) {
            //当前是字幕的编辑状态，点击到的是动图，不响应点击事件
            return;
        }else if ((pasterController.pasterType == AliyunPasterEffectTypeCaption || pasterController.pasterType == AliyunPasterEffectTypeSubtitle)&& _editSouceClickType != AliyunEditSouceClickTypeCaption){
...
- (void)forceFinishLastEditPasterView {
    ...
    if (editPasterController.pasterType == AliyunPasterEffectTypeSubtitle) {
        ...
        [self animateWithObject:editPasterController animation:_tmpActionType];
...
- (void)completeButtonClicked {
    ...
    if (_currentTextInputView) {
        ...
        if (_currentEditPasterType == AliyunPasterEffectTypeSubtitle &&[_currentTextInputView getText].length <=0)
        {//如果是纯文字模式，并且文字输入为空的情况下直接销毁输入框
...
{
    if ([self.editor getTimeFilter] != 3 || actionType == TextActionTypeNull || actionType == TextActionTypeClear) {//倒播
        [_currentTextInputView setTextAnimationType:actionType];
    }else{
//        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"提示" message:@"倒播时不支持添加字幕动画效果" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
//        [alert show];
        [MBProgressHUD showMessage:@"倒播时不支持添加字幕动画效果" inView:self.view];
    }
}
...
- (void)keyboardFrameChanged:(CGRect)rect animateDuration:(CGFloat)duration {
    ...
    if (self.currentEditPasterType == AliyunPasterEffectTypeSubtitle) {
        tabItems = @[@(TabBarItemTypeKeboard),
                     @(TabBarItemTypeColor),
                     @(TabBarItemTypeFont),
    ...
    [self.tabController presentTabContainerViewInSuperView:self.view height:hieght duration:duration tabItems:tabItems];
...
- (void)pasterButtonClicked {
    ...
    [self enterEditWithActionType:AliyunEditSouceClickTypePaster animationCompletion:nil];
...
- (void)subtitleButtonClicked {
    ...
    [self enterEditWithActionType:AliyunEditSouceClickTypeCaption animationCompletion:nil];
...
- (void)effectButtonClicked{
    ...
    [self enterEditWithActionType:AliyunEditSouceClickTypeEffect animationCompletion:nil];
}
...
- (void)translationButtonCliked{
    if([self.clipConstructor mediaClips].count<2){
        [MBProgressHUD showMessage:@"一段视频无法添加转场" inView:self.view];
        return;
    }
    
    if (!self.transitionRetention.isFirstEdit) {
        //如果不是第一次编辑转场则获取上次转场状态
        NSMutableArray *copyIcons = [[NSMutableArray alloc]initWithArray:_transitionRetention.transitionIcons copyItems:YES];
        
        NSMutableArray *copyCovers = [[NSMutableArray alloc]initWithArray:_transitionRetention.transitionCovers copyItems:YES];
        [self.transitionView setIcons:copyIcons];
        [self.transitionView setCovers:copyCovers];
    }
    [self enterEditWithActionType:AliyunEditSouceClickTypeTranslation animationCompletion:nil];
}
...
- (void)updateDrawRect:(CGRect)drawRect {
    self.paintView.frame = CGRectMake(drawRect.origin.x, drawRect.origin.y+SafeTop, drawRect.size.width, drawRect.size.height-120);
}

#pragma mark - AliyunEffectFilter2ViewDelegate - 滤镜 AliyunEffectFilterViewDelegate - MV 公有
/**
 应用滤镜特效的效果
 */
...
- (void)noApplyButtonClick{
    ...
    if (_editSouceClickType == AliyunEditSouceClickTypeEffect) {
        for (int i = 0; i<self.animationFilters.count; i++) {
            [self.editor removeAnimationFilter:self.animationFilters[i]];
        }
        ...
        self.animationFilters = [self.storeAnimationFilters mutableCopy];
...
#pragma mark - AliyunEffectFilter2ViewDelegate - 现有滤镜里的特效
...
//            }
            ...
            if (_processAnimationFilter.endTime < _processAnimationFilter.startTime || _processAnimationFilter.endTime > [self.player getDuration]) {
...
-(void)pasterBottomView:(AliyunPasterBottomBaseView *)bottomView didSelectedPasterModel:(AliyunEffectPasterInfo *)pasterInfo{
    ...
    if (editStaut && editPasterController) {
        [self deletePasterController:editPasterController];
    }
    else{
        [self forceFinishLastEditPasterView];
    }
...
- (void)onClickFontWithFontInfo:(AliyunEffectFontInfo *)font {
    ...
    textInputView.center = CGPointMake(CGRectGetMidX(self.movieView.bounds), CGRectGetMidY(self.movieView.bounds) - 50);
...
//添加字幕动画
- (void)animateWithObject:(AliyunPasterController *)pasterController animation:(TextActionType)type {
    ...
    switch (type) {
        ...
        case TextActionTypeMoveLeft: {
            ...
            moveAction.fromePoint = CGPointMake(CGRectGetWidth(self.movieView.frame), pasterController.pasterPosition.y);
        ...
        case TextActionTypeMoveRight: {
            ...
            moveAction.startTime = [pasterController pasterStartTime];
            moveAction.duration = 1;
            moveAction.fromePoint = CGPointMake(CGRectGetWidth(pasterController.pasterFrame)*-1, pasterController.pasterPosition.y);
            moveAction.toPoint = pasterController.pasterPosition;
            [effectPaster runAction:moveAction];
        }
            break;
        case TextActionTypeMoveTop: {
            ...
            moveAction.startTime = [pasterController pasterStartTime];
            moveAction.duration = 1;
            moveAction.fromePoint = CGPointMake(pasterController.pasterPosition.x, CGRectGetHeight(self.editZoneView.frame));
            moveAction.toPoint = pasterController.pasterPosition;
            [effectPaster runAction:moveAction];
        }
            break;
        case TextActionTypeMoveDown: {
            ...
            moveAction.startTime = [pasterController pasterStartTime];
            moveAction.duration = 1;
            moveAction.fromePoint = CGPointMake(pasterController.pasterPosition.x, CGRectGetHeight(pasterController.pasterFrame) *-1);
            moveAction.toPoint = pasterController.pasterPosition;
            [effectPaster runAction:moveAction];
        }
            break;
        ...
        case TextActionTypeFade:{
            ...
            alphaAction_out.startTime = [pasterController pasterStartTime];
            alphaAction_out.duration = 1;
...
- (void)textInputViewEditCompleted {
    ...
    if (editPasterController) {//当前有正在编辑的动图控制器，则更新
        ...
        editPasterController = [self.pasterManager addSubtitle:[_currentTextInputView getText] bounds:inputViewBounds startTime:range.startTime duration:range.duration];
    ...
    [self makePasterControllerBecomeEditStatus:editPasterController];
    _tmpActionType = [_currentTextInputView actionType];
    [self destroyInputView];
}
...
//
//    if (_textActionType != TextActionTypeNull || _textActionType != TextActionTypeClear) {
////        [MBProgressHUD showMessage:@"倒播时不支持添加字幕动画效果" inView:self.view];
////        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"提示" message:@"倒播时不支持添加字幕动画效果" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
////        [alert show];
//    }
    ...
    if (!_invertAvailable) {
        ...
        if (!_invertAvailable) {
            ...
            [self.compressManager compressWithSourcePath:inputPath outputPath:outputPath outputSize:[asset aliyunNaturalSize] success:^{
                ...
                [weakself.player play];//这里必须调用self.player的play 要不原始视频流时间和当前播放时间会反
...
-(void)applyButtonClickCovers:(NSArray<AliyunTransitionCover *> *)covers andIcons:(NSArray<AliyunTransitionIcon *> *)icons transitionInfo:(NSDictionary *)transitionInfo{
    ...
    self.transitionRetention.transitionIcons = [[NSMutableArray alloc]initWithArray:icons copyItems:YES];
    [self quitEditWithActionType:_editSouceClickType CompletionHandle:nil];
}
...
-(void)transitionCancelButtonClickTransitionInfo:(NSDictionary *)transitionInfo{
    ...
    for (NSString *key in self.transitionRetention.lastTransitionInfo) {
        TransitionType lastType = (TransitionType)[self.transitionRetention.lastTransitionInfo[key] intValue];
