...
- (void)fetchListData {
    ...
    [AliyunResourceRequestManager requestWithEffectTypeTag:self.effectType success:^(NSArray *resourceListArray) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [weakSelf.effectMoreView.tableView reloadData];
        });
    ...
    if (cell.buttontType == EffectTableViewButtonDownload) {
        ...
        [manager addDownloadTask:task progress:^(CGFloat progress) {
            ...
            if (error) {
                ...
                weakSelf.dataArray[indexPath.row] = newModel;
                [weakSelf.dbHelper insertDataWithEffectResourceModel:newModel];
//                if (newModel.effectType == AliyunEffectTypePaster || newModel.effectType == AliyunEffectTypeCaption) {
//                    if (self.effectMoreCallback) {
//                        self.effectMoreCallback(newModel);
//                    }
//                }else{
                    if (weakSelf.effectMoreCallback) {
                        weakSelf.effectMoreCallback(nil);
                    }
//                }
