...
- (void)btnClick:(UIButton *)clickButton{
    ...
    } else if([_cellModel.title isEqualToString:@"视频比例"]){
    ...
    } else if([_cellModel.title isEqualToString:@"裁剪模式"]){
        if (clickButton.tag == 0) {
            //裁剪
...
- (void)layoutSubviews {
    ...
    _titleLabel.frame = CGRectMake(30, 15, 72, 20);
    if ([self.reuseIdentifier isEqualToString:@"cellInput"]) {
        ...
        _titleLabel.frame = CGRectMake(30, 30, 72, 20);
    }else {
