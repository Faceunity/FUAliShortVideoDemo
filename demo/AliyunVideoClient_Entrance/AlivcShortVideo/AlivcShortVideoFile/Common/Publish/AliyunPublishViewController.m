...
@interface AliyunPublishViewController () <AliyunPublishTopViewDelegate, AliyunIExporterCallback, UITextFieldDelegate,UIAlertViewDelegate>
...
- (void)tryExport{
    if (!_finished) {
        ...
        BOOL isSuccess = [[AliyunPublishService service] exportWithTaskPath:_taskPath outputPath:_config.outputPath];
        if (!isSuccess) {
            AlivcAlertView *alertView = [[AlivcAlertView alloc]initWithAlivcTitle:nil message:@"合成失败,请返回重试" delegate:self cancelButtonTitle:nil confirmButtonTitle:@"确定"];
            [alertView show];
        }
    }
}
